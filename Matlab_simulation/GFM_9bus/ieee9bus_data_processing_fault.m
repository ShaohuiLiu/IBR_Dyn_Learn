...
nl = 8; % location
ni = 20; % duration
nj = 1; % magnitude, disabled in fault


load_all = 0;
if load_all < 1 
    Filename='results160fault_9bus.mat';
    ...
    for i = 1 : ni
        for j = 1 : nj
            Filename=sprintf('Fault_%d_Duration_%d%.mat', l, i);
            load(strcat(datapath,Filename));
            disp(strcat(Filename,' loaded.'))
            
            t0{20*(l-1) + i} = sim_out.out_v.Time;
            out_SM_V_terminal0{20*(l-1) + i} = sim_out.out_SM_V_terminal.Data;
            out_SM_states0{20*(l-1) + i} = sim_out.out_SM_states.Data;
            out_GFM2_V_terminal0{20*(l-1) + i} = sim_out.out_GFM2_V_terminal.Data;
            out_GFM2_states0{20*(l-1) + i} = sim_out.out_GFM2_states.Data;
            out_GFM3_V_terminal0{20*(l-1) + i} = sim_out.out_GFM3_V_terminal.Data;
            out_GFM3_states0{20*(l-1) + i} = sim_out.out_GFM3_states.Data;
            out_p0{20*(l-1) + i} = sim_out.out_p.Data;
            out_freq0{20*(l-1) + i} = sim_out.out_freq.Data;
            out_v0{20*(l-1) + i} = sim_out.out_v.Data;
            
            % {20*(l-1) + i} = ans.Pm.Data;
        end
    end
    end
end
...
figure
for i = 1 : ni*nj
plot(t0{1}(t_fault(1):end),out_freq0{i}(t_fault(1):end,:),'LineWidth',2);
hold on
end
grid on
...
%% save to csv (SM)
filename0 = 'ieee9bus_160fault_SM_';
...
%% save to csv (GFM)
filename0 = 'ieee9bus_160fault_GFM3_';
