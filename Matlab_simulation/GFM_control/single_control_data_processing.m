...
datapath = '/mnt/Data1/Dyn_learning/Matlab_simulation/GFM_control/Sim_Outputs/dVOC_single_control200/';
...
if load_all < 1 
    ...
    for i = 1 : ni
        % for j = 1 : nj
            ...
            t0{(l-1)*10 + (i)*1} = sim_out.out_v.Time;
            out_bus9_V_terminal0{(l-1)*ni + (i)*1} = sim_out.out_bus9_V_terminal.Data;
            out_SM_V_terminal0{(l-1)*ni + (i)*1} = sim_out.out_SM_V_terminal.Data;
            out_SM_states0{(l-1)*ni + i} = sim_out.out_SM_states.Data;
            % out_GFM2_V_terminal0{(l-1)*10 + i} = sim_out.out_GFM2_V_terminal.Data;
            % out_GFM2_states0{(l-1)*10 + i} = sim_out.out_GFM2_states.Data;
            out_GFM3_V_terminal0{(l-1)*ni + i} = sim_out.out_GFM3_V_terminal.Data;
            out_GFM3_states0{(l-1)*ni + i} = sim_out.out_GFM3_states.Data;
            out_GFM3_control0{(l-1)*ni + i} = sim_out.out_control.Data;
            out_p0{(l-1)*ni + i} = sim_out.out_p.Data;
            out_freq0{(l-1)*ni + i} = sim_out.out_freq.Data;
            out_v0{(l-1)*ni + i} = sim_out.out_v.Data;
            t1{(l-1)*ni + i} = sim_out.out_GFM3_V_terminal.Time;
...
filename = strcat(datapath,filename0,'vbus_0327.csv');
writematrix(Vbus_data,filename)
filename = strcat(datapath,filename0,'theta_0327.csv');
writematrix(Theta_date,filename)

filename = strcat(datapath,filename0,'time_0327.csv');
writematrix(t_data,filename)


filename = strcat(datapath,filename0,'freq_data_0327.csv');
writematrix(freq_data,filename)


filename = strcat(datapath,filename0,'delta_data_0327.csv');
writematrix(Delta_data,filename)


filename = strcat(datapath,filename0,'vd_0327.csv');
writematrix(Vd_data,filename)

filename = strcat(datapath,filename0,'vq_0327.csv');
writematrix(Vq_data,filename)

filename = strcat(datapath,filename0,'pm_0327.csv');
writematrix(Pm_data,filename)

filename = strcat(datapath,filename0,'vref_0327.csv');
